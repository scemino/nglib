...
proc genEnums(output: var string) =
  let file = readFile("src/nglib/private/cimgui/generator/output/structs_and_enums.json")
  ...
  for name, obj in data["enums"].pairs:
    ...
    for data in obj:
      ...
      let splits = dataName.split("_")
      dataName = splits[splits.len-1]
...
proc genTypeDefs(output: var string) =
  ...
  let file = readFile("src/nglib/private/cimgui/generator/output/typedefs_dict.json")
  let data = file.parseJson()
...
proc genProcs(output: var string) =
  let file = readFile("src/nglib/private/cimgui/generator/output/definitions.json")
  let data = file.parseJson()
...
proc igGenerate*() =
  ...
  writeFile("src/nglib/imgui.nim", output)
